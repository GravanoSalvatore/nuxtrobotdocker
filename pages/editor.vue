...
export default {
  setup() {
    const channelStore = useChannelStore() // Подключаем Pinia Store
    const message = ref('') // Поле ввода сообщения
    const uploadedFiles = ref([]) // Загруженные файлы
    ...
    // Получаем botToken и activeChannelId из Pinia
  ...
  if (!activeChannelId.value) {
    console.error('Активный канал не выбран!');
    return;
  }
  ...
  try {
    ...
    const payload = {
      chat_id: activeChannelId.value, // Указываем активный канал
      text: message.value,           // Текст сообщения
    };

    // Отправляем запрос к API Telegram
    const response = await axios.post(
      `https://api.telegram.org/bot${botToken.value}/sendMessage`,
      payload, // Передаём JSON-параметры
      {
        headers: {
